...
func _ready():
	...
	if restart_button: 
		...
		print("[GameOverMenu] _ready: Сигнал RestartButton.pressed подключен.")
	if exit_menu_button: 
		exit_menu_button.pressed.connect(_on_ExitMenuButton_pressed)
		print("[GameOverMenu] _ready: Сигнал ExitMenuButton.pressed подключен.")
...
func show_screen(final_score: int):
	...
	if not is_inside_tree():
		...
		return # Нельзя ставить на паузу дерево, если узел не в нем
...
func _on_RestartButton_pressed():
	print("[GameOverMenu] _on_RestartButton_pressed: Кнопка 'Перезапустить уровень' нажата.")
	hide() 
	Input.set_mouse_mode(previous_mouse_mode) 
	get_tree().paused = false 
	print("[GameOverMenu] _on_RestartButton_pressed: Игра снята с паузы.")
	
	GameManager.reset_game_state() 
	print("[GameOverMenu] _on_RestartButton_pressed: GameManager.reset_game_state() вызван.")
	var error = get_tree().reload_current_scene()
	if error != OK:
		print("[GameOverMenu] _on_RestartButton_pressed: !!! ОШИБКА перезапуска уровня: " + error_string(error))
	else:
		print("[GameOverMenu] _on_RestartButton_pressed: Уровень перезапускается.")


func _on_ExitMenuButton_pressed():
	print("[GameOverMenu] _on_ExitMenuButton_pressed: Кнопка 'Выход в главное меню' нажата.")
	hide()
	Input.set_mouse_mode(previous_mouse_mode)
	get_tree().paused = false
	print("[GameOverMenu] _on_ExitMenuButton_pressed: Игра снята с паузы.")
	
	print("[GameOverMenu] _on_ExitMenuButton_pressed: Выход из игры...")
	get_tree().quit()

# Эта функция _unhandled_input обычно не нужна для экрана Game Over,
# так как он не закрывается по Esc (в отличие от меню паузы).
# Если она вам нужна, раскомментируйте и адаптируйте.
# func _unhandled_input(event: InputEvent):
# 	if get_tree().paused and visible and event.is_action_pressed("ui_cancel"): # ui_cancel обычно Esc
# 		print("[GameOverMenu] _unhandled_input: Нажат ui_cancel (Esc) во время Game Over.")
# 		# _on_ExitMenuButton_pressed() # Например, выполнить действие выхода
